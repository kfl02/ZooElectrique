...
public class ControlDevice {
	...
	private final Map<String, ControlSlider> slidersByName = new HashMap<>();

	/**
	 * list containing the buttons on the device
	 */
	private final List<ControlButton> buttons = new ArrayList<ControlButton>();
	private final Map<String, ControlButton> buttonsByName = new HashMap<>();
	 ...
	 * @param i_controller
	 */
	ControlDevice(final Controller i_controller){
	...
	private void setupDevice(){
		...
		for (int i = 0; i < components.length; i++){
			...
			if(components[i].isAnalog()){
				...
				slidersByName.put(input.getName(), (ControlSlider)input);
			} else {
				if(components[i].getIdentifier() == Component.Identifier.Axis.POV){
					input = new ControlHat(components[i]);
				}else{
					input = new ControlButton(components[i]);
				}
				buttons.add((ControlButton)input);
                buttonsByName.put(input.getName(), (ControlButton)input);
			} 
			if(input != null){
	 ...
	 * @return
	 */
	public Controller.Type getTypeID(){
	...
	public ControlSlider getSlider(final int i_sliderNumb){ 
		return sliders.get(i_sliderNumb); 
	} 
	...
	public ControlSlider getSlider(final String i_sliderName){
	    if(slidersByName.containsKey(i_sliderName)) {
	        return slidersByName.get(i_sliderName);
	    } else {
	...
	public void setTolerance(final float i_tolerance){
		for (int i = 0; i < sliders.size(); i++)
			sliders.get(i).setTolerance(i_tolerance);
	}
	...
	public ControlButton getButton(final String i_buttonName){
	    if(buttonsByName.containsKey(i_buttonName)) {
	        return buttonsByName.get(i_buttonName);
	    } else {
	...
	public ControlHat getHat(final String i_hatName){
        if(buttonsByName.containsKey(i_hatName) && buttonsByName.get(i_hatName) instanceof ControlHat) {
            return (ControlHat)buttonsByName.get(i_hatName);
        } else {
	...
	public void plug(Callback callback, EventType type, int button) {
	    getButton(button).plug(callback, type);
	}

    public void plug(Callback callback, EventType type, String button) {
        getButton(button).plug(callback, type);
    }
	...
	public boolean equals(Object d){
        ...
        if(!(d instanceof ControlDevice)) {
            return false;
        }
        
        ControlDevice dev = (ControlDevice)d;

        if(buttons.size() != dev.buttons.size() 
                || sliders.size() != dev.sliders.size() 
                || rumblers.length != dev.rumblers.length
                || name.compareToIgnoreCase(dev.name) != 0) {
            return false;
        }
